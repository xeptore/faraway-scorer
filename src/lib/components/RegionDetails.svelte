<script lang="ts">
  ...
  let {
    ...
    position,
    onclose,
  }: { result: RegionScoreResult; position: number; onclose: () => void } = $props()
  ...
  >
    ...
    <div class="detail-block">
      <div class="detail-title-row">
        <h3>Prerequisite</h3>
        <span
          class:success={result.prerequisite.satisfied}
          class:error={!result.prerequisite.satisfied}
        >
      ...
      {:else}
        <div
          class="detail-table"
          role="table"
          aria-label="Resource prerequisites"
        >
          <div
          ...
          {#each result.prerequisite.requirements as requirement}
            ...
            >
              ...
              <span
                role="cell"
                class:error={!requirement.satisfied}
              >
                <strong>{requirement.visible}</strong>
              </span>
            </div>
          {/each}
        </div>
    ...
    <div class="detail-block">
      ...
      {#if result.calculation.type === 'fixed'}
        <p class="calculation-line">
          Fixed reward <strong>{result.calculation.fame} Fame</strong>
        </p>
      {:else if result.calculation.type === 'perCount'}
        <div class="detail-list">
          {#each result.calculation.matches as match}
            <div>
              <span>{targetLabel(match.target)}</span>
              <strong>{match.count}</strong>
            </div>
          {/each}
        </div>
        <p class="calculation-line">
          {result.calculation.totalCount} matching {pluralize(
            result.calculation.totalCount,
            'item',
          )} × {result.calculation.famePerItem} Fame
        </p>
      {:else}
        <div class="detail-list">
          {#each result.calculation.members as member}
            <div>
              <span>{targetLabel(member.target)}</span>
              <strong>{member.count}</strong>
            </div>
          {/each}
        </div>
        <p class="calculation-line">
          {result.calculation.setCount} complete {pluralize(result.calculation.setCount, 'set')} × {result
            .calculation.famePerSet} Fame
        </p>
      {/if}
    </div>
    ...
    {#if !result.prerequisite.satisfied}
      <p class="failed-note">
        The calculation is shown for reference, but this Region scores 0 because its prerequisite is
        not met.
      </p>
    {/if}
    ...
    <div class="detail-total">
      <span>Total</span>
      <strong>{result.fame} Fame</strong>
    </div>
  </div>
</div>
