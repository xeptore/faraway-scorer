<script lang="ts">
  ...
  function initialSetupPlayers(): SetupPlayer[] {
    return [
      createSetupPlayer(playerNamePlaceholders[0]),
      createSetupPlayer(playerNamePlaceholders[1]),
    ]
  }
  ...
  const scores = $derived.by(() => {
    ...
    for (const player of game.players) {
      const sequence = player.regions.map((id, position) =>
        validationErrors.has(regionFieldKey(player.id, position)) ||
        rawErrors[regionFieldKey(player.id, position)]
          ? null
          : id,
      ) as RegionSequence
  ...
  const fameByPlayer = $derived(
    new Map(
      game?.players.map((player) => [player.id, scores.get(player.id)?.totalFame ?? 0]) ?? [],
    ),
  )
  const standings = $derived(game ? rankPlayers(game.players, fameByPlayer) : [])
  const allComplete = $derived(
    game?.players.every((player) => scores.get(player.id)?.complete) ?? false,
  )
  const activePlayer = $derived(
    game?.players.find((player) => player.id === game?.activePlayerId) ?? game?.players[0] ?? null,
  )
...
{#if view === 'resume' && game}
  <main class="start-shell">
    <section class="start-card">
      ...
      <p class="intro">
        Continue where you left off, with every Region and Sanctuary saved on this device.
      </p>
      <div class="resume-summary">
        {#each game.players as player}
          <div>
            ...
            <strong>
              {scoreRegionSequence(player.sanctuary, player.regions, cardLookup).enteredCount}/8
            </strong>
          </div>
        {/each}
      </div>
  ...
  <main class="start-shell">
    <section class="start-card setup-card">
      ...
      <button
        ...
        type="button"
        onclick={startGame}
        disabled={setupPlayers.some((player) => !player.name.trim())}
      >
  ...
  <div class="score-app">
    <header class="app-header">
      ...
      <div class="header-score">
        <span>{activeScore.complete ? 'Total fame' : 'Current fame'}</span>
        <strong>{activeScore.totalFame}</strong>
      </div>
      <button
        class="menu-button"
        type="button"
    ...
    >
      {#each game.players as player}
        ...
        >
          <span>{player.name}</span>
          <small>{scores.get(player.id)?.enteredCount ?? 0}/8</small>
        </button>
      {/each}
    </nav>
    ...
    <main class="score-content">
      {#if REGION_CARDS.length === 0}
        <aside class="dataset-notice">
          ...
          <span>
            Add the official definitions to <code>src/lib/data/regions.ts</code>
            ; undefined numbers are safely rejected until then.
          </span>
        </aside>
      {/if}
      ...
      <section class="section-card sanctuary-card">
        <div class="section-heading">
          <div>
            <p class="eyebrow">Always visible</p>
            <h2>Sanctuaries</h2>
          </div>
          <label class="fame-input">
            <span>Sanctuary fame</span>
            <input
              type="number"
              min="0"
              inputmode="numeric"
              value={activePlayer.sanctuary.fame}
              oninput={(event) =>
                (activePlayer.sanctuary.fame = Math.max(
                  0,
        ...
        <details>
          <summary>
            <span>Scoring contributions</span>
            <small>Resources, biomes & more</small>
          </summary>
          <div class="counter-group">
            ...
            {#each RESOURCES as resource}
              <Counter
                ...
                value={activePlayer.sanctuary.resources[resource]}
                onchange={(value) =>
                  setNonNegative(() => (activePlayer.sanctuary.resources[resource] = value))}
              />
            {/each}
          </div>
          <div class="counter-group">
            ...
            {#each BIOMES as biome}
              <Counter
                label={BIOME_LABELS[biome]}
                value={activePlayer.sanctuary.biomes[biome]}
                onchange={(value) =>
                  setNonNegative(() => (activePlayer.sanctuary.biomes[biome] = value))}
              />
            {/each}
          </div>
          <div class="counter-group two-column-counters">
            <Counter
              label="Clues"
              value={activePlayer.sanctuary.clues}
              onchange={(value) => setNonNegative(() => (activePlayer.sanctuary.clues = value))}
            />
            <Counter
              label="Nighttime cards"
              value={activePlayer.sanctuary.nighttimeCards}
              onchange={(value) =>
                setNonNegative(() => (activePlayer.sanctuary.nighttimeCards = value))}
            />
          </div>
        </details>
      </section>
      ...
      <section class="journey-section">
        <div class="journey-heading">
          <div>
            ...
            <h2>Score in reverse</h2>
          </div>
          <span>{activeScore.enteredCount} of 8 entered</span>
        </div>
        <p class="journey-help">
          Start with the last Region played. Each valid card scores immediately.
        </p>
        ...
        <div class="journey-list">
          {#each REGION_POSITIONS as playedPosition, position}
            ...
            <div
              class:error-row={Boolean(error)}
              class:complete-row={Boolean(result)}
              class="region-row"
            >
              <div class="position-marker">
                <strong>{playedPosition}</strong>
                <span>
                  {playedPosition === 1
                    ? 'st'
                    : playedPosition === 2
              ...
              {#if result}
                <button
                  class="row-score"
                  type="button"
                  aria-label={`Explain ${result.fame} Fame for Region ${result.regionId}`}
                  onclick={() => (selectedDetail = { result, position: playedPosition })}
                >
                  <strong>{result.fame > 0 ? '+' : ''}{result.fame}</strong>
                  <span>›</span>
                </button>
              {:else}
              ...
              {/if}
              {#if error}<p
                  id={`error-${activePlayer.id}-${position}`}
                  class="field-error"
                >
                  {error}
                </p>{/if}
      ...
      >
        <div>
          <span>Region fame</span>
          <strong>{activeScore.regionFame}</strong>
        </div>
        <div>
          <span>Sanctuary fame</span>
          <strong>{activeScore.sanctuaryFame}</strong>
        </div>
    ...
    <aside
      class="score-tray"
      class:expanded={trayExpanded}
    >
      {#if trayExpanded}
        <div
          class="tray-backdrop"
          role="presentation"
          onclick={() => (trayExpanded = false)}
        ></div>
        <section
        ...
        >
          ...
          <div class="sheet-header">
            <div>
              ...
              <h2 id="standings-title">Current standings</h2>
            </div>
            <button
              bind:this={standingsCloseButton}
              class="icon-button"
              type="button"
          ...
          <div class="standings-list">
            {#each standings as entry}
              ...
              >
                ...
                <small>{scores.get(entry.player.id)?.enteredCount ?? 0}/8</small>
                <strong>{entry.fame}</strong>
              </button>
            {/each}
          </div>
          ...
          {#if !allComplete}<p class="tray-note">
              Complete all eight Regions for every player to see final results.
            </p>{/if}
        </section>
      {/if}
      <div class="tray-bar">
        ...
        <div class="tray-players">
          {#each game.players as player}
            ...
            >
              <span>{player.name}</span>
              <strong>{scores.get(player.id)?.totalFame ?? 0}</strong>
              <small>{scores.get(player.id)?.enteredCount ?? 0}/8</small>
            </button>
          {/each}
        </div>
...
{#if showNewGameConfirm}
  <div
    ...
    role="presentation"
    onclick={(event) => event.target === event.currentTarget && (showNewGameConfirm = false)}
  >
    <div
...
{:else if offlineReady}
  ...
  >
    <div>
      ...
      <span>This scorekeeper will work without a connection.</span>
    </div>
  </aside>
{/if}
